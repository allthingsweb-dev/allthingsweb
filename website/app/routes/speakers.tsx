...
import { toYearStr } from '~/modules/datetime.ts';
import { getMetaTags, mergeMetaTags } from '~/modules/meta.ts';
import { type loader as rootLoader } from '~/root.tsx';
import { speakersLoader as loader } from '~/modules/speakers/loader.server.ts';
            ...
            {speakersWithTalks.map((speaker) => (
              <Card key={speaker.id} className='flex flex-col'>
                <CardHeader className='flex flex-row items-center gap-4 pb-2'>
                  <img
                    src={getImageSrc(speaker.profileImageUrl, {
                      width: 200,
                      height: 200,
                      fit: 'cover',
                    })}
