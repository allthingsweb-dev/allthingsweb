...
export const createSessionManager = ({ mainConfig, redirect }: Deps): SessionManager => {
  ...
  const createUserSession: SessionManager['createUserSession'] = async (headers = new Headers()) => {
  ...
  const requireCanonicalSession: SessionManager['requireCanonicalSession'] = async (request) => {
