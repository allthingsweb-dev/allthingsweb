...
export async function GET(request: NextRequest) {
  ...
  try {
    ...
    requestUrl.searchParams.forEach((value, key) => {
      if (
        ["live", "table", "handle", "offset", "cursor", "where"].includes(key)
      ) {
    ...
    // Set the table name
    electricUrl.searchParams.set("table", "neon_auth.users_sync");

    // Only get active users (not deleted)
    electricUrl.searchParams.set("where", "deleted_at IS NULL");
