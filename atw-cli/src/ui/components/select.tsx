...
export function Select<T>({
  ...
  const optionsToDisplay = (() => {
    ...
    return options.slice(
      selectedIndex - 1,
      selectedIndex - 1 + maxOptionsToDisplay,
    );
  })();
  const lastDisplayedIndex = options.findIndex(
    (option: Option<T>) =>
      option === optionsToDisplay[optionsToDisplay.length - 1],
  );
  const overflowItem =
    lastDisplayedIndex < options.length - 1
      ? options[lastDisplayedIndex + 1]
      : null;
        ...
        return (
          <Box flexDirection="row" marginY={1} key={index}>
            <Box width={1} marginRight={2}>
              <Text color={highlightColor}>{isSelected ? ">" : ""}</Text>
            </Box>
            <Box>
              {option.render ? (
                option.render(option.label, option.value, isSelected)
              ) : (
